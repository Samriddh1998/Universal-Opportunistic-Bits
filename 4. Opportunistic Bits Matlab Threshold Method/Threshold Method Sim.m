%% Script is faster
% This is the actual algorithm that works properly - as per the paper. the
% removal of elements is random 
% Stack -  Number of new elements that enter the storage after the current element is input.
% Stack2 - takes note of how early/late the packets are thrown away.
ps = [];
for K = 4:4
N = 2^K;
count = zeros(1,N);
delay = 60000; % Max delay of 50 units is acceptable.
dlCtr = zeros(N,1);
NosIter = 1000000;
...
%     offset(M) = M*(M-1)/2;
    ...
    for i = 1:NosIter % Total transmissions      
        ...
        if(space == M) % Always Happens -- :)
            if(Storage(1,2) == 1) % Single packet disposal => random one thrown

                  [oldestTime] = (Time2(randi(M),1));
                  if(oldestTime <= tnxno) % swapping
...
for i = 1:N 
    hold on
    plotitnow(i) = max(BigStack(i,100:end));
end
plot(plotitnow,'o-')
%%
figure
